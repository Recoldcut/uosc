...
function render()
	...
	if state.is_idle and not Manager.disabled.idle_indicator then
		...
		local center_x, center_y, icon_size = display.width / 2, display.height / 2, math.max(smaller_side / 6, 56)
		ass:icon(center_x, center_y - icon_size / 4, icon_size, 'fiber_manual_record', {
			-- color = fg, opacity = config.opacity.idle_indicator,
			color = bg, opacity = config.opacity.idle_indicator,
		})
		ass:txt(center_x, center_y + icon_size / 2, 8, t('Drop files or URLs to play here'), {
			size = icon_size / 5, color = fg, opacity = config.opacity.idle_indicator*0.6,
		})
	end
